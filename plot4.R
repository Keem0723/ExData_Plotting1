...
# PNG Plot 4: A Lattice Plot of Time-series Line Graphs of Global Active Power, Energy Sub Metering, Global Reactive Power, & Voltage With Respect To Day
DateTimeVect <- with(dt0, as.POSIXlt(paste(dt0$Date, dt0$Time), format ="%d/%m/%Y %H:%M:%S")) #Coercing date-rime data from character to POSIXlt

png("plot4.png",width = 480,height = 480) #Launching PNG graphic device

par(mfcol = c(2,2)) #Using par function to set parameters for the graphic

plot(DateTimeVect,dt0$Global_active_power,type = "l",xlab = "", ylab = "Global Active Power") #Creating the line graph according to specification

plot(DateTimeVect,dt0$Sub_metering_1,type = "l",xlab = "", ylab = "Energy sub metering") #Creating the line graph according to specification
lines(DateTimeVect,dt0$Sub_metering_2,col = "red") #Adding line graph according to specification
lines(DateTimeVect,dt0$Sub_metering_3,col = "blue") #Adding line graph according to specification
legend("topright",lty = c(1,1,1),col = c("black","red","blue"),c("Sub_metering_1","Sub_metering_2","Sub_metering_3")) #Adding legend to plot according to specification 

plot(DateTimeVect,dt0$Voltage,type = "l",xlab = "datetime", ylab = "Voltage") #Creating the line graph according to specification

plot(DateTimeVect,dt0$Global_reactive_power,type = "l",xlab = "datetime", ylab = "Global_reactive_power")#Creating the line graph according to specification
